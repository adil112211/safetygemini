...
const authMiddleware = (req, res, next) => {
    try {
        ...
        if (validateInitData(initData)) {
            ...
            if (!userParam) {
                return res.status(400).json({ message: 'Bad Request: User data not found in initData' });
            }
            req.user = JSON.parse(userParam);
            next();
        } else {
            return res.status(403).json({ message: 'Forbidden: Invalid data signature. Possible missing BOT_TOKEN on server.' });
        }
    } catch (error) {
